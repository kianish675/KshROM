![KshROM Banner](readme-res/banner.png)

<p align="center">KshROM is a fork of UN1CA-A70 for the Galaxy A70</p>
<p align="center">
  <a href="https://github.com/ksh675/KshROM/discussions">🚀 Discussions</a>
  •
  <a href="https://t.me/KshROM">💬 Telegram</a>
</p>



## DISCLAIMER
Your warranty is now **void**.  
I am not responsible for bricked devices, dead SD cards, or anything else that goes wrong.  


## FEATURES
- Some Custom Ringtones
- Extra Mods (Outdoor Mode, etc.)
- Now Brief (only works for now bar atp)
- Selinux Full Support
- Galaxy AI (Photo Editor) Support
- Mid-end Animations
- CSC Mods (Network Speed in Status Bar, 5GHz Hotspot)
- Battery life Improvments
- Debloated from Unnecessary Applications, etc.
- Multi-User Support
- DeKnoxed
- KnoxPatch Included in various System Frameworks
- Bluetooth Library Patch Included
- EroFS Partitions
- All software features from A73 5G
- A bit of Optimizations sprinkled on top
- Network Optimizations
- Adaptive Brightness Support
- RIL Funtional
- Bootloader spoofed


---

## KNOWN BUGS
- Bluetooth/A2DP

## SOURCES
- [UN1CA Project](https://github.com/salvogiangri/UN1CA)  
- UN1CA-A70  
- Kernel Source Code  

## CREDITS
KshROM:
- [Tise](https://github.com/tisenu100) for his work on bringing OneUI 8 to the A70, and for his UN1CA-A70 repo containing essential patches, mods, and more, which serves as the parent of this fork.
- [Ocin4ever](https://github.com/Ocin4ever) for the Paradigm Mod, his co-authoring on some smali patches
- [ExtremeXT](https://github.com/ExtremeXT) for his patches and mods, Support of Bluetooth Library Patch for One UI 7.
- [nookgit](https://github.com/nookgit12) for his emotional support, getting me into porting, and also being a Contributor for the rom.
- [rtd1250](https://github.com/rtd1250) for the main Galaxy A70 support device trees, kernel support
- [3arthur6](https://github.com/3arthur6) for Bluetooth Library Patch.
- [Pascua](https://github.com/pascua28) For Improving the SM6150 kernel & his upstreams (PrimeKernel)

UN1CA Team:
- [Salvo](https://github.com/salvogiangri) project founder and developer of the build system
- [ShaDisNX255](https://github.com/ShaDisNX255)
- [DavidArsene](https://github.com/DavidArsene)
- [paulowesll](https://github.com/paulowesll)
- [Simon1511](https://github.com/Simon1511)
- [ananjaser1211](https://github.com/ananjaser1211)
- [iDrinkCoffee](https://github.com/iDrinkCoffee-TG)
- [RisenID](https://github.com/RisenID)

---

## COMMUNITY
💬 Have a question? Join our Telegram Group



# Licensing:
KshROM abides to the GPLv3 license like the original UN1CA project

# Kernel Version
Kernel Version: 4.14.190